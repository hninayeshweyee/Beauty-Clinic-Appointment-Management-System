...
if(isset($_GET['delete_id'])) {
    ...
    $delete = mysqli_query($connect, "DELETE FROM contact_messages WHERE contactID = '$id'");
    if($delete) {
        echo "<script>window.alert('Message Deleted Successfully')</script>";
        echo "<script>window.location='Contact_Manage.php'</script>";
    }
}
...
<head>
  ...
  <title>Manage Inquiries | GlowWave Clinic</title>
  ...
  <style>
    ...
    .btn-delete { color: #d9534f; cursor: pointer; transition: 0.3s; border: none; background: none; }
    .btn-delete:hover { color: #c9302c; transform: scale(1.1); }
    .message-truncate { max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; }
  </style>
</head>
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 bg-gradient-dark" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link text-white active bg-custom-brown" href="../pages/Contact_Manage.php">
                    <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Inquiries</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Manage User Inquiries</h6>
        </nav>
      ...
      <div class="card my-4">
        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
          <div class="bg-custom-brown shadow-primary border-radius-lg pt-4 pb-3">
            <h6 class="text-white ps-3">Contact Messages</h6>
          </div>
        </div>
        <div class="card-body px-0 pb-2">
          <div class="table-responsive p-0">
            <table class="table align-items-center mb-0">
              ...
              <tbody>
                <?php
                $res = mysqli_query($connect, "SELECT * FROM contact_messages ORDER BY contactID DESC");
                ...
                <tr>
                  ...
                  <td>
                    <span class="text-xs text-secondary message-truncate"><?= $msg ?></span>
                    <a href="javascript:;" class="text-info text-xxs font-weight-bold" 
                       data-bs-toggle="modal" data-bs-target="#viewMsg<?= $row['contactID'] ?>">Read More</a>
                  </td>
                  <td class="align-middle text-center">
                    <span class="text-secondary text-xs font-weight-bold"><?= date('M d, Y', strtotime($row['submitted_at'])) ?></span>
                  </td>
                  <td class="align-middle text-center">
                    <a href="Contact_Manage.php?delete_id=<?= $row['contactID'] ?>" 
                       onclick="return confirm('Delete this inquiry?');" class="btn-delete">
                      <i class="material-icons">delete</i>
                    </a>
                  </td>
                </tr>

                <div class="modal fade" id="viewMsg<?= $row['contactID'] ?>" tabindex="-1" role="dialog" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title font-weight-normal">Message from <?= htmlspecialchars($row['name']) ?></h5>
                        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                        </button>
                      </div>
                      <div class="modal-body">
                        <p class="text-sm"><strong>Subject:</strong> <?= htmlspecialchars($row['subject']) ?></p>
                        <hr class="dark horizontal">
                        <p class="text-muted"><?= nl2br($msg) ?></p>
                      </div>
